...
public class BleManager {
    ...
    // 广义搜索模式：true=不过滤特征码，扫描所有设备；false=仅扫描心率服务
    ...
    /** 筛选模式回调 — 用于 connectAndFilterServices */
        ...
        manualScanCb = new ScanCallback() {
            ...
            public void onScanResult(int callbackType, ScanResult result) {
                ...
                if (deviceMap.containsKey(addr)) {
                    ...
                    // 合并新扫描到的 service UUID（有些设备可能广播多个包）
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            try {
                ScanSettings settings = new ScanSettings.Builder()
                    ...
                    .build();
                if (broadScan) {
                    // 广义搜索：不设 filter，扫描所有 BLE 设备
                    leScanner.startScan(null, settings, cb);
                } else {
                    // 标准模式：只扫描包含心率服务 UUID 的设备
    ...
    /**
     * 筛选模式：连接设备 → 遍历所有 service/characteristic
     * → 匹配含 heart/hr/rate 等关键词的特征 → 订阅通知
     */
    ...
    private final BluetoothGattCallback filterGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            // 没有匹配到心率特征时，尝试订阅所有有 Notify 属性的特征
            if (filterMatched == 0) {
                for (BluetoothGattService svc : services) {
