...
public class MainActivity extends FragmentActivity {
    ...
    private final BleManager.BleListener bleListener = new BleManager.BleListener() {
        ...
        public void onConnecting(String name) {
            runOnUiThread(() -> {
                if (connectFragment != null) connectFragment.updateStatus(name, "❤️ 连接中...");
        ...
        public void onConnected(String name, String address) {
            ...
            runOnUiThread(() -> {
                if (connectFragment != null) connectFragment.updateStatus(name, "已连接");
        ...
        public void onDisconnected(String name) {
            runOnUiThread(() -> {
                ...
                // 【修改】不再强制隐藏悬浮窗，仅显示 "--" 离线状态
                // 主播的OBS悬浮窗不会因为蓝牙断开就消失
