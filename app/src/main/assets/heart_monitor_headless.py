# -*- coding: utf-8 -*-
"""
❤️ 心迹 v3.0 — 无头版（无需桌面环境，纯 Web 浏览器操作）
...
# ═════════════════════ 启动 ═════════════════════
print("❤️ 心迹 v3.0 无头版")
print(f"📥 浏览器打开: http://{ip()}:{UNIFIED_PORT}")
start_server()
print(f"\n✅ 服务已启动！在浏览器中访问:")
print(f"   http://{ip()}:{UNIFIED_PORT}")
print(f"📱 手机推送: http://{ip()}:{UNIFIED_PORT}/api/hr")
print(f"📺 OBS 透明: http://{ip()}:{UNIFIED_PORT}/?transparent=1")
print("\n按 Ctrl+C 停止服务")
