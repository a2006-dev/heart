...
public class BroadcastActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btnSharePython).setOnClickListener(v -> showShareDialog());
    ...
    private void showShareDialog() {
        String[] options = {"📦 heart_client.exe（双击即用，无需安装）", "🐍 heart_monitor.py（需安装 Python）"};
        new androidx.appcompat.app.AlertDialog.Builder(this)
            .setTitle("分享电脑端客户端")
            .setItems(options, (dialog, which) -> {
                if (which == 0) shareExeClient();
                else sharePythonClient();
            })
            .setNegativeButton("取消", null)
            .show();
    }

    private void shareExeClient() {
        try {
            // 先从 assets 复制 exe 到缓存目录
            java.io.File cacheDir = new java.io.File(getCacheDir(), "share");
            if (!cacheDir.exists()) cacheDir.mkdirs();
            java.io.File destFile = new java.io.File(cacheDir, "heart_client.exe");
            try (java.io.InputStream is = getAssets().open("heart_client.exe");
            ...
            shareIntent.putExtra(Intent.EXTRA_TEXT, "❤️ 心迹 - 电脑端接收程序（exe版）\n\n双击即用，无需安装 Python\n\n访问地址: " + tvAccessURL.getText().toString());
            shareIntent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
            startActivity(Intent.createChooser(shareIntent, "分享 exe 客户端"));
        } catch (Exception e) {
